                            ...
                            ${catProducts.map(p => `
                                <div class="menu-card-item" data-variants="${p.variants ? JSON.stringify(p.variants).replace(/"/g, '&quot;') : '[]'}">
                                    <div class="menu-item-header">
                                        <h3 class="menu-item-name">${p.name.replace(/\s*\((.*?)\)/g, ' <span class="inline-desc">($1)</span>')} ${p.description ? (p.description.length <= 40 ? `<span class="inline-desc">(${p.description})</span>` : '') + `<em style="display:none;">${p.description}</em>` : ''}</h3>
